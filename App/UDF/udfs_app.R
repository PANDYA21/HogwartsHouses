## ---------------------------------------------------------------------------------------------------------- ##
## User Defined  Functions - script 2
## required during the app usage
## Bhaumik Pandya
...
getFinDf <- function(In.age, In.sex, In.weight, In.lev, In.ptype, In.err, In.head, In.stroke,
  ...
  } else if(grepl("low[ ]*speed", In.err, ignore.case = T)){
  ...
  if (grepl("no|No|NO|nO", In.weight, ignore.case = T)){
    ...
    if(is.na(as.numeric(In.weight))){
      if(grepl("light", tolower(In.weight))){
        In.weight <- "light (under 260g)"
      } else if (grepl("heavy", tolower(In.weight))){
  ...
  if(In.head == ""){
    ...
    if(grepl("large", tolower(In.head))){
      In.head <- "large (over 660 cm²)"
    } else if(grepl("small", tolower(In.head))){
      In.head <- "small (under 630 cm²)"
    } else {
      In.head <- "middle (630-660 cm²)"
    }
...
getUserDetails <- function(path.userLogs = "User_logs/"){
  # usrs.csv <- list.files(pattern = "*.csv", path = path.userLogs)
  usrs.csv <- list.files(pattern = "*.csv", recursive = T)
